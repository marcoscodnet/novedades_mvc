...
function confirmDeleteCategoryAjax(message, title, url, divId){
   
	jConfirm(message,  title, function(r) {
        if (r) {
        	URLWithAjaxCall(url, divId);
            return false
        } else {
            return false;
        }
...
function selected_categories() {
	//alert(getAllPageItemsSelected_cd_category());
	//startSearch("#sitebody");
	jQuery("#sitebody").load( "doAction?action=sending_process&cd_newsletter=" + getSelected_1()+"&categories=" + getAllPageItemsSelected_cd_category() ,'',function() {
		window.location = "doAction?action=list_newsletters";
	});
}

function selected_contacts() {
	//alert(getAllPageItemsSelected_cd_contact_category());
	
	jQuery("#sitebody").load( "doAction?action=contactcategory_process&cd_category=" + getSelected_1()+"&contacts=" + getAllPageItemsSelected_cd_contact_category(),'',function() {
		window.location = "doAction?action=list_categories";
	});
}
...
function URLWithAjaxCall(url, divId){
    jQuery.ajax({
        url:url,
